...
Các tham số chung (đồng bộ giữa Client/Server) chỉnh trong `config.py` ở root dự án:
`CONNECTION_TIMEOUT`, `MAX_FILE_SIZE_MB`, `CHUNK_SIZE`, `BUFFER_SIZE`.
File `server/server_config.py` sẽ import các giá trị này. Tránh chỉnh lệch ở nơi khác.
...
- Các tham số dùng chung (timeout, giới hạn kích thước file, chunk/buffer) được quản lý tại `config.py` (root dự án) và được cả client/server import. `server/server_config.py` chỉ đóng vai trò bọc/tham chiếu lại các giá trị này.
